...
async function ContractList({ token }: { token: string }) {
  ...
  return (
    <>
      {contracts.map(contract => (
        <Link href={`/contracts/${contract.id || (contract as any)._id}`} key={contract.id || (contract as any)._id}>
          <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-6 hover:border-blue-500/50 hover:bg-gray-800 transition-all cursor-pointer group h-full flex flex-col justify-between">
            <div>
...
function ContractListFallback() {
  return (
    <>
      {[1, 2, 3, 4, 5, 6].map(i => <ContractCardSkeleton key={i} />)}
    </>
  );
}
    ...
    <div className="min-h-screen bg-gray-900 text-white p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <Suspense fallback={<ContractListFallback />}>
